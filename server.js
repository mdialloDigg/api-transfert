...
form{background:#fff;padding:20px;border-radius:10px;box-shadow:0 0 10px rgba(0,0,0,.1);width:300px}
...
header{padding:10px;background:#007bff;color:#fff;display:flex;justify-content:space-between;align-items:center}
...
@media(max-width:600px){table,thead,tbody,th,td,tr{display:block}th{display:none}td{border:none;position:relative;padding-left:50%}td::before{position:absolute;left:10px;width:45%;white-space:nowrap}}
</style>
</head>
<body>
<header>
...
async function loadTransferts(){
  ...
  data.transferts.forEach(t=>{
    ...
    row.innerHTML=\`<td>\${t.code}</td><td>\${t.senderFirstName}</td><td>\${t.senderLastName}</td><td>\${t.senderPhone}</td><td>\${t.originLocation}</td><td>\${t.destinationLocation}</td><td>\${t.amount}</td><td>\${t.currency}</td><td>\${t.retired?'Oui':'Non'}</td><td><button onclick="edit('\${t._id}')">Modifier</button><button onclick="suppr('\${t._id}')">Supprimer</button></td>\`;
...
// --- FORMULAIRE ---
app.get('/transferts/new', auth, (req,res)=>res.redirect('/transferts/form'));
app.get('/transferts/edit/:id', auth, (req,res)=>res.redirect('/transferts/form?id='+req.params.id));
...
app.post('/api/transferts', auth, async (req,res)=>{ const t=new Transfert(req.body); await t.save(); res.json({success:true}); });
app.put('/api/transferts/:id', auth, async (req,res)=>{ await Transfert.findByIdAndUpdate(req.params.id, req.body); res.json({success:true}); });
app.delete('/api/transferts/:id', auth, async (req,res)=>{ await Transfert.findByIdAndDelete(req.params.id); res.json({success:true}); });
