...
app.get('/login',(req,res)=>{
  ...
  <style>
  body{margin:0;font-family:Arial;background:#ff8c42;display:flex;justify-content:center;align-items:center;height:100vh;}
  ...
  button{background:#ff8c42;color:white;font-weight:bold;cursor:pointer;}
  </style></head><body>
...
app.get('/dashboard', requireLogin, async(req,res)=>{
  ...
  // =================== Transferts ===================
  html+=`<h3>Transferts</h3><button onclick="openTransfertModal()">➕ Nouveau</button>
  <table>
  <tr><th>Code</th><th>Expéditeur</th><th>Origine</th><th>Destinataire</th><th>Destination</th><th>Montant</th><th>Frais</th><th>Reçu</th><th>Devise</th><th>Mode</th><th>Status</th><th>Actions</th></tr>`;
  transferts.forEach(t=>{
    ...
    <td>${t.amount}</td><td>${t.fees}</td><td>${t.received}</td><td>${t.currency}</td>
  ...
  // =================== Stocks ===================
  html+=`<h3>Stocks</h3><button onclick="openStockModal()">➕ Nouveau</button>
  <table>
  <tr><th>Code</th><th>Expéditeur</th><th>Phone</th><th>Destination</th><th>Phone</th><th>Montant</th><th>Devise</th><th>Actions</th></tr>`;
  stocks.forEach(s=>{
    html+=`<tr>
    <td>${s.code}</td><td>${s.sender}</td><td>${s.senderPhone}</td>
    <td>${s.destination}</td><td>${s.destinationPhone}</td>
  ...
  // =================== Clients ===================
  html+=`<h3>Clients</h3><button onclick="openClientModal()">➕ Nouveau</button>
  <table>
  ...
  // =================== Rates ===================
  html+=`<h3>Taux</h3><button onclick="openRateModal()">➕ Nouveau</button>
  <table>
...
<input id="s_senderPhone" placeholder="Téléphone">
<input id="s_destination" placeholder="Destination">
<input id="s_destinationPhone" placeholder="Téléphone">
...
// =================== Transfert ===================
function openTransfertModal(id=null){currentTransfertId=id;document.getElementById('transfertModal').style.display='flex';if(!id){t_code.value='';t_origin.value='';t_sender.value='';t_senderPhone.value='';t_destination.value='';t_receiver.value='';t_receiverPhone.value='';t_amount.value='';t_fees.value='';t_received.value='';return;}
...
// =================== Stock ===================
function openStockModal(id=null){currentStockId=id;document.getElementById('stockModal').style.display='flex';if(!id){s_code.value='';s_sender.value='';s_senderPhone.value='';s_destination.value='';s_destinationPhone.value='';s_amount.value='';return;}
...
// =================== Client ===================
function openClientModal(id=null){currentClientId=id;document.getElementById('clientModal').style.display='flex';if(!id){c_firstName.value='';c_lastName.value='';c_phone.value='';c_email.value='';c_kyc.value='false';return;}
fetch('/client/'+id).then(r=>r.json()).then(c=>{c_firstName.value=c.firstName;c_lastName.value=c.lastName;c_phone.value=c.phone;c_email.value=c.email;c_kyc.value=c.kycVerified;});}
...
// =================== Rate ===================
function openRateModal(id=null){currentRateId=id;document.getElementById('rateModal').style.display='flex';if(!id){r_from.value='';r_to.value='';r_rate.value='';return;}
fetch('/rate/'+id).then(r=>r.json()).then(rate=>{r_from.value=rate.from;r_to.value=rate.to;r_rate.value=rate.rate;});}
...
// =================== API ROUTES ===================
// Transfert CRUD
app.get('/transfert/:id',requireLogin, async(req,res)=>res.json(await Transfert.findById(req.params.id)));
...
app.post('/transfert/delete',requireLogin, async(req,res)=>{ await Transfert.findByIdAndDelete(req.body.id); res.json({ok:true}); });
app.post('/transfert/retirer',requireLogin, async(req,res)=>{ const t = await Transfert.findById(req.body.id); t.retired=true; t.retraitHistory.push({date:new Date(),mode:req.body.mode}); await t.save(); res.json({ok:true}); });

// Stock CRUD
app.get('/stock/:id',requireLogin, async(req,res)=>res.json(await Stock.findById(req.params.id)));
app.post('/stock/new',requireLogin, async(req,res)=>{ const {_id,...data}=req.body; if(_id) await Stock.findByIdAndUpdate(_id,data); else { data.code=await generateUniqueCode(); await new Stock(data).save(); } res.json({ok:true}); });
app.post('/stock/delete',requireLogin, async(req,res)=>{ await Stock.findByIdAndDelete(req.body.id); res.json({ok:true}); });

// Client CRUD
app.get('/client/:id',requireLogin, async(req,res)=>res.json(await Client.findById(req.params.id)));
app.post('/client/new',requireLogin, async(req,res)=>{ const {_id,...data}=req.body; if(_id) await Client.findByIdAndUpdate(_id,data); else await new Client(data).save(); res.json({ok:true}); });
app.post('/client/delete',requireLogin, async(req,res)=>{ await Client.findByIdAndDelete(req.body.id); res.json({ok:true}); });

// Rate CRUD
app.get('/rate/:id',requireLogin, async(req,res)=>res.json(await Rate.findById(req.params.id)));
app.post('/rate/new',requireLogin, async(req,res)=>{ const {_id,...data}=req.body; if(_id) await Rate.findByIdAndUpdate(_id,data); else await new Rate(data).save(); res.json({ok:true}); });
app.post('/rate/delete',requireLogin, async(req,res)=>{ await Rate.findByIdAndDelete(req.body.id); res.json({ok:true}); });

// =================== EXPORTS ===================
// TODO: add PDF/Excel export code if needed
