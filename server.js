...
/******************************************************************
 * APP TRANSFERT + STOCKS + CLIENTS + TAUX – VERSION TOTALE
 ******************************************************************/
require('dotenv').config();
const express = require('express');
const mongoose = require('mongoose');
...
app.use(express.urlencoded({ extended: true }));
app.use(express.json());
app.use(session({ secret: process.env.SESSION_SECRET || 'transfert-secret-full', resave: false, saveUninitialized: true }));
...
app.get('/login',(req,res)=>{
  res.send(`<html><head><meta name="viewport" content="width=device-width, initial-scale=1">
  <style>
  body{margin:0;font-family:Arial,sans-serif;background:#1e1e2f;display:flex;justify-content:center;align-items:center;height:100vh;color:white;}
  .login-container{background:#2c2c3e;padding:40px;border-radius:15px;box-shadow:0 10px 30px rgba(0,0,0,0.5);width:90%;max-width:360px;text-align:center;}
  .login-container h2{margin-bottom:30px;font-size:28px;color:#ff8c42;}
  .login-container input{width:100%;padding:15px;margin:10px 0;border:none;border-radius:10px;font-size:16px;}
  .login-container button{padding:15px;width:100%;border:none;border-radius:10px;font-size:16px;background:#ff8c42;color:white;font-weight:bold;cursor:pointer;transition:0.3s;}
  .login-container button:hover{background:#e67300;}
  </style></head><body>
  <div class="login-container">
...
app.get('/dashboard', requireLogin, async(req,res)=>{
  ...
  let html=`<html><head><meta name="viewport" content="width=device-width, initial-scale=1">
  <style>
  body{margin:0;font-family:Arial,sans-serif;background:#1e1e2f;color:white;padding:20px;}
  h2,h3{color:#ff8c42;}
  table{border-collapse:collapse;width:100%;margin-bottom:20px;background:#2c2c3e;border-radius:10px;overflow:hidden;}
  ...
  button.edit{background:#007bff;color:white;}
  button.delete{background:#dc3545;color:white;}
  button.retirer{background:#ffc107;color:white;}
  .modal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);justify-content:center;align-items:center;}
  .modal-content{background:#2c2c3e;padding:20px;border-radius:10px;width:90%;max-width:500px;color:white;}
  input,select{width:100%;padding:10px;margin:5px 0;border-radius:8px;border:none;background:#1e1e2f;color:white;}
  </style></head><body>
  <h2>📊 Dashboard</h2><a href="/logout" style="color:#ff8c42;">Déconnexion</a>`;

  // ===== TRANSFERTS =====
  html+=`<h3>Transferts</h3><button class="add" onclick="openTransfertModal()">+ Nouveau Transfert</button>
  <table><tr><th>Code</th><th>Expéditeur</th><th>Destinataire</th><th>Montant</th><th>Frais</th><th>Reçu</th><th>Devise</th><th>Status</th><th>Actions</th></tr>`;
  transferts.forEach(t=>{
    html+=`<tr>
    <td>${t.code}</td>
  ...
  // ===== STOCKS =====
  html+=`</table><h3>Stocks</h3><button class="add" onclick="openStockModal()">+ Nouveau Stock</button>
  <table><tr><th>Code</th><th>Expéditeur</th><th>Destination</th><th>Montant</th><th>Devise</th><th>Actions</th></tr>`;
  stocks.forEach(s=>{
    html+=`<tr>
    <td>${s.code}</td><td>${s.sender}</td><td>${s.destination}</td>
    <td>${s.amount}</td><td>${s.currency}</td>
    <td><button class="edit" onclick="editStock('${s._id}')">✏️</button>
    <button class="delete" onclick="deleteStock('${s._id}')">❌</button></td></tr>`;
  });

  // ===== CLIENTS =====
  html+=`</table><h3>Clients KYC</h3><button class="add" onclick="openClientModal()">+ Nouveau Client</button>
  <table><tr><th>Nom</th><th>Prénom</th><th>Téléphone</th><th>Email</th><th>KYC</th><th>Actions</th></tr>`;
  clients.forEach(c=>{
    html+=`<tr><td>${c.lastName}</td><td>${c.firstName}</td><td>${c.phone}</td><td>${c.email}</td>
    <td>${c.kycVerified?'Oui':'Non'}</td>
    <td><button class="edit" onclick="editClient('${c._id}')">✏️</button>
    <button class="delete" onclick="deleteClient('${c._id}')">❌</button></td></tr>`;
  });

  // ===== RATES =====
  html+=`</table><h3>Taux / Devise</h3><button class="add" onclick="openRateModal()">+ Nouveau Taux</button>
  <table><tr><th>De</th><th>Vers</th><th>Taux</th><th>Actions</th></tr>`;
  rates.forEach(r=>{
    html+=`<tr><td>${r.from}</td><td>${r.to}</td><td>${r.rate}</td>
    <td><button class="edit" onclick="editRate('${r._id}')">✏️</button>
    <button class="delete" onclick="deleteRate('${r._id}')">❌</button></td></tr>`;
  });
  ...
  function closeModal(){document.getElementById('modal').style.display='none';currentId=null;}
  async function post(url,data){return fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(r=>r.json());}
  async function del(url){return fetch(url,{method:'DELETE'}).then(r=>r.json());}

  // === TRANSFERTS ===
  function openTransfertModal(){openModal('Transfert','<input id="t_senderFirstName" placeholder="Expéditeur Prénom"><input id="t_senderLastName" placeholder="Expéditeur Nom"><input id="t_receiverFirstName" placeholder="Destinataire Prénom"><input id="t_receiverLastName" placeholder="Destinataire Nom"><input id="t_amount" type="number" placeholder="Montant"><input id="t_fees" type="number" placeholder="Frais"><input id="t_currency" placeholder="Devise"><button onclick="saveTransfert()">Enregistrer</button>');}
  async function saveTransfert(){const data={senderFirstName:document.getElementById('t_senderFirstName').value,senderLastName:document.getElementById('t_senderLastName').value,receiverFirstName:document.getElementById('t_receiverFirstName').value,receiverLastName:document.getElementById('t_receiverLastName').value,amount:parseFloat(document.getElementById('t_amount').value),fees:parseFloat(document.getElementById('t_fees').value),received:parseFloat(document.getElementById('t_amount').value)-parseFloat(document.getElementById('t_fees').value),currency:document.getElementById('t_currency').value};if(currentId)data._id=currentId;await post('/transferts/save',data);location.reload();}
  function editTransfert(id){currentId=id;location.reload();}
  function deleteTransfert(id){if(confirm('Supprimer ce transfert?')){del('/transferts/delete/'+id).then(()=>location.reload());}}
  function retirerTransfert(id){post('/transferts/retirer/'+id,{mode:'ESPECE'}).then(()=>location.reload());}

  // === STOCKS ===
  function openStockModal(){openModal('Stock','<input id="s_sender" placeholder="Expéditeur"><input id="s_destination" placeholder="Destination"><input id="s_amount" type="number" placeholder="Montant"><input id="s_currency" placeholder="Devise"><button onclick="saveStock()">Enregistrer</button>');}
  async function saveStock(){const data={sender:document.getElementById('s_sender').value,destination:document.getElementById('s_destination').value,amount:parseFloat(document.getElementById('s_amount').value),currency:document.getElementById('s_currency').value};if(currentId)data._id=currentId;await post('/stocks/save',data);location.reload();}
  function editStock(id){currentId=id;location.reload();}
  function deleteStock(id){if(confirm('Supprimer stock?')){del('/stocks/delete/'+id).then(()=>location.reload());}}

  // === CLIENTS ===
  function openClientModal(){openModal('Client','<input id="c_firstName" placeholder="Prénom"><input id="c_lastName" placeholder="Nom"><input id="c_phone" placeholder="Téléphone"><input id="c_email" placeholder="Email"><select id="c_kyc"><option value=true>Oui</option><option value=false>Non</option></select><button onclick="saveClient()">Enregistrer</button>');}
  async function saveClient(){const data={firstName:document.getElementById('c_firstName').value,lastName:document.getElementById('c_lastName').value,phone:document.getElementById('c_phone').value,email:document.getElementById('c_email').value,kycVerified:document.getElementById('c_kyc').value==='true'};if(currentId)data._id=currentId;await post('/clients/save',data);location.reload();}
  function editClient(id){currentId=id;location.reload();}
  function deleteClient(id){if(confirm('Supprimer client?')){del('/clients/delete/'+id).then(()=>location.reload());}}

  // === RATES ===
  function openRateModal(){openModal('Taux','<input id="r_from" placeholder="De"><input id="r_to" placeholder="Vers"><input id="r_rate" type="number" step="0.01" placeholder="Taux"><button onclick="saveRate()">Enregistrer</button>');}
  async function saveRate(){const data={from:document.getElementById('r_from').value,to:document.getElementById('r_to').value,rate:parseFloat(document.getElementById('r_rate').value)};if(currentId)data._id=currentId;await post('/rates/save',data);location.reload();}
  function editRate(id){currentId=id;location.reload();}
  function deleteRate(id){if(confirm('Supprimer taux?')){del('/rates/delete/'+id).then(()=>location.reload());}}
  </script></body></html>`;
  res.send(html);
});
