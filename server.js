/******************************************************************
 * APP TRANSFERT – TOUT EN UN SERVEUR
 ******************************************************************/
...
app.get('/transferts/list', requireLogin, (req,res)=>{
  ...
  <select id="currency"><option value="">Toutes devises</option>${currencies.map(c=>`<option value="${c}">${c}</option>`).join('')}</select>
  <select id="destination"><option value="">Toutes destinations</option>${locations.map(d=>`<option value="${d}">${d}</option>`).join('')}</select>
  <button onclick="loadData()">🔍 Filtrer</button>
  <a href="/logout">🚪 Déconnexion</a>
  <a href="/transferts/excel">📊 Excel</a><a href="/transferts/word">📝 Word</a>
  <table><thead><tr>
  <th>Code</th><th>Type</th><th>Expéditeur</th><th>Origine</th><th>Destinataire</th>
  <th>Montant</th><th>Frais</th><th>Reçu</th><th>Devise</th><th>Status</th><th>Actions</th></tr></thead><tbody id="tbody"></tbody></table>
...
app.get('/transferts/excel', requireLogin, async(req,res)=>{
  ...
  sheet.columns = [{header:'Code', key:'code'},{header:'Type', key:'userType'},{header:'Expéditeur', key:'sender'},{header:'Origine', key:'origin'},{header:'Destinataire', key:'receiver'},{header:'Montant', key:'amount'},{header:'Frais', key:'fees'},{header:'Reçu', key:'recovery'},{header:'Devise', key:'currency'},{header:'Statut', key:'status'}];
...
app.get('/transferts/word', requireLogin, async(req,res)=>{
  ...
  transferts.forEach(t=>{doc.addSection({children:[new Paragraph({children:[new TextRun({text:`Code: ${t.code} | Exp: ${t.senderFirstName} ${t.senderLastName} | Dest: ${t.receiverFirstName} ${t.receiverLastName} | Montant: ${t.amount} ${t.currency} | Frais: ${t.fees} | Reçu: ${t.recoveryAmount} | Statut: ${t.retired?'Retiré':'Non retiré'}`})]})]});});
...
app.get('/transferts/print/:id', requireLogin, async(req,res)=>{
  ...
  res.send(`<html><body><div><p>Code: ${t.code}</p><p>Exp: ${t.senderFirstName} ${t.senderLastName}</p><p>Dest: ${t.receiverFirstName} ${t.receiverLastName}</p><p>Montant: ${t.amount} ${t.currency}</p><p>Frais: ${t.fees}</p><p>Reçu: ${t.recoveryAmount}</p><p>Status: ${t.retired?'Retiré':'Non retiré'}</p></div><script>window.print()</script></body></html>`);
});
