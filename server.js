...
app.get('/dashboard', requireLogin, async(req,res)=>{
  ...
  <tr><th>Date</th><th>Code</th><th>Expéditeur</th><th>Destination</th><th>Montant</th><th>Devise</th><th>Actions</th></tr>`;
  ...
  // Rates Table
  html+=`<h3>Taux de Change</h3><table>
  ...
  // Modals HTML (Transfert / Stock / Client / Rate)
  html+=`
...
fetch('/transfert/'+id).then(r=>r.json()).then(t=>{
t_code.value=t.code; t_origin.value=t.originLocation; t_sender.value=t.senderFirstName; t_senderPhone.value=t.senderPhone;
t_destination.value=t.destinationLocation; t_receiver.value=t.receiverFirstName; t_receiverPhone.value=t.receiverPhone;
t_amount.value=t.amount; t_fees.value=t.fees; t_received.value=t.received; t_currency.value=t.currency; t_recoveryMode.value=t.recoveryMode;
});}
...
function saveTransfert(){
  const amount=parseFloat(t_amount.value)||0; const fees=parseFloat(t_fees.value)||0;
  postData('/transfert/new',{
    _id:currentTransfertId, originLocation:t_origin.value, senderFirstName:t_sender.value,
    senderPhone:t_senderPhone.value, destinationLocation:t_destination.value, receiverFirstName:t_receiver.value,
    receiverPhone:t_receiverPhone.value, amount, fees, received:amount-fees, currency:t_currency.value,
...
/* STOCK */
function openStockModal(id=null){ currentStockId=id; stockModal.style.display='flex';
if(!id){ s_code.value=''; s_sender.value=''; s_senderPhone.value=''; s_destination.value=''; s_destinationPhone.value=''; s_amount.value=''; return; }
fetch('/StockHistory/'+id).then(r=>r.json()).then(s=>{
s_code.value=s.code; s_sender.value=s.sender; s_senderPhone.value=s.senderPhone; s_destination.value=s.destination;
s_destinationPhone.value=s.destinationPhone; s_amount.value=s.amount; s_currency.value=s.currency;
});}
function closeStockModal(){ stockModal.style.display='none'; currentStockId=null; }
function saveStock(){
  const data={ sender:s_sender.value, senderPhone:s_senderPhone.value, destination:s_destination.value, destinationPhone:s_destinationPhone.value, amount:parseFloat(s_amount.value), currency:s_currency.value };
  if(currentStockId) data._id=currentStockId;
  postData('/StockHistory/new',data).then(()=>location.reload());
}
function deleteStock(id){ if(confirm('Supprimer ?')) postData('/StockHistory/delete',{id}).then(()=>location.reload()); }
...
app.post('/transfert/new', async(req,res)=>{
  try{ const data=req.body; if(data._id) await Transfert.findByIdAndUpdate(data._id,data,{new:true});
  else{ data.code=await generateUniqueCode(); data.userType='Client'; await new Transfert(data).save(); } res.json({success:true}); }
...
// StockHistory
app.get('/StockHistory/:id', requireLogin, async(req,res)=>{ res.json(await StockHistory.findById(req.params.id)); });
app.post('/StockHistory/new', requireLogin, async(req,res)=>{
  try{
    let stock;
    if(req.body._id){
...
app.post('/StockHistory/delete', requireLogin, async(req,res)=>{
  const stock=await StockHistory.findById(req.body.id);
  if(stock){ await StockHistory.create({action:'SUPPRESSION', stockId:stock._id, ...stock.toObject()});
...
app.get('/export/excel', requireLogin, async(req,res)=>{
  const workbook=new ExcelJS.Workbook(); const sheet=workbook.addWorksheet('Transferts');
  sheet.columns=[{header:'Code',key:'code',width:10},{header:'Expéditeur',key:'sender',width:20},{header:'Destinataire',key:'receiver',width:20},{header:'Montant',key:'amount',width:10},{header:'Frais',key:'fees',width:10},{header:'Reçu',key:'received',width:10},{header:'Devise',key:'currency',width:10},{header:'Status',key:'status',width:10}];
