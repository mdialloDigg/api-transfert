...
app.get('/login', (req,res)=>{
  res.send(`<form method="post"><input name="username"><input name="password" type="password"><button>Login</button></form>`);
});

app.post('/login', async (req,res)=>{
  const user = await AuthUser.findOne({ username:req.body.username });
  if(!user) return res.send('Utilisateur inconnu');
  if(!await bcrypt.compare(req.body.password, user.password)) return res.send('Mot de passe incorrect');
