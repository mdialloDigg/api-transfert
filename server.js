/******************************************************************
 * APP TRANSFERT – DASHBOARD FINAL AVEC AJAX / TOTAUX / EXPORT
 ******************************************************************/
...
<table>
<thead><tr>
<th>Code</th><th>Type</th><th>Expéditeur</th><th>Origine</th><th>Destinataire</th><th>Montant</th><th>Frais</th><th>Reçu</th><th>Devise</th><th>Status</th><th>Actions</th>
</tr></thead>
...
async function loadData(){
  ...
  const resp = await fetch('/transferts/data?search='+encodeURIComponent(s)+'&status='+encodeURIComponent(status)+'&currency='+encodeURIComponent(currency)+'&destination='+encodeURIComponent(destination));
...
function retirer(id){const m=prompt('Mode de retrait: Espèces, Orange Money, Wave');if(m)fetch('/transferts/retirer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,mode:m})}).then(loadData);}
...
app.get('/transferts/data', requireLogin, async(req,res)=>{
  ...
  transferts = transferts.filter(t=>{
    return (!s || t.senderFirstName.toLowerCase().includes(s) || t.receiverFirstName.toLowerCase().includes(s) || t.code.toLowerCase().includes(s)) &&
...
app.get('/transferts/excel', requireLogin, async(req,res)=>{
  ...
  sheet.columns=[
    ...
    {header:'Expéditeur', key:'sender'}, {header:'Origine', key:'originLocation'},
...
app.get('/transferts/word', requireLogin, async(req,res)=>{
  ...
  transferts.forEach(t=>{
    doc.addSection({children:[
      new Paragraph({children:[new TextRun(`Code: ${t.code} | Expéditeur: ${t.senderFirstName} ${t.senderLastName} | Destinataire: ${t.receiverFirstName} ${t.receiverLastName} | Montant: ${t.amount} ${t.currency} | Status: ${t.retired?'Retiré':'Non retiré'}`)])}
    ]});
  });
...
app.get('/transferts/print/:id', requireLogin, async(req,res)=>{
  ...
  res.send(`<html><head><meta name="viewport" content="width=device-width, initial-scale=1"><title>Imprimer</title></head><body>
  ...
  <p><strong>Reçu:</strong> ${t.recoveryAmount}</p>
