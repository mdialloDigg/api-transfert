...
app.get('/transferts/list', requireLogin, async(req,res)=>{
  ...
  let grouped = {};
  filtered.forEach(t=>{ if(!grouped[t.destinationLocation]) grouped[t.destinationLocation]=[]; grouped[t.destinationLocation].push(t); });

  let html=`<html><head><meta name="viewport" content="width=device-width, initial-scale=1"><style>
body{margin:0;font-family:Arial;background:#f0f4f8;padding:15px;}
h2,h3{color:#2c7be5;margin:5px 0;}
...
button{margin:2px;padding:6px 10px;border:none;border-radius:6px;color:white;}
button.delete{background:#dc3545;}
button.print{background:#17a2b8;}
button.modify{background:#28a745;}
form.inline{display:inline;}
input,select{padding:6px;border-radius:6px;border:1px solid #ccc;margin-right:5px;}
...
<a href="/menu">⬅ Menu</a> | <a href="/transferts/form">➕ Nouveau</a> | <a href="/transferts/pdf">📄 PDF</a><hr>
...
<button type="submit">🔍 Rechercher</button>
  ...
  for(let dest in grouped){
    html += `<h3>Destination: ${dest}</h3><div class="container"><table>
