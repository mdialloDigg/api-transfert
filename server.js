 ...
 * COMPLET : Dashboard avec modals CRUD et recherche instantanée
 ******************************************************************/
...
app.get('/dashboard', requireLogin, async(req,res)=>{
  ...
  <table><thead><tr>
  <th>Code</th><th>Origine</th><th>Expéditeur</th><th>Téléphone</th><th>Destination</th><th>Destinataire</th><th>Téléphone</th>
  <th>Montant</th><th>Frais</th><th>Reçu</th><th>Devise</th><th>Status</th><th>Actions</th>
  </tr></thead><tbody id="transfert-body"></tbody></table>

  <h3>Stocks</h3>
  ${p.ecriture?'<button onclick="openStockModal()">➕ Nouveau Stock</button>':''}
  <table><thead><tr>
  <th>Code</th><th>Expéditeur</th><th>Téléphone</th><th>Destination</th><th>Téléphone</th><th>Montant</th><th>Devise</th><th>Actions</th>
  </tr></thead><tbody id="stock-body"></tbody></table>

  <h3>Clients KYC</h3>
  ${p.ecriture?'<button onclick="openClientModal()">➕ Nouveau Client</button>':''}
  <table><thead><tr>
  <th>Nom</th><th>Prénom</th><th>Téléphone</th><th>Email</th><th>KYC</th><th>Actions</th>
  </tr></thead><tbody id="client-body"></tbody></table>

  <h3>Taux de Change</h3>
  ${p.ecriture?'<button onclick="openRateModal()">➕ Nouveau Taux</button>':''}
  <table><thead><tr>
  <th>De</th><th>Vers</th><th>Rate</th><th>Actions</th>
  </tr></thead><tbody id="rate-body"></tbody></table>
...
async function loadTables() {
  ...
  try{
    // Transferts
    const resT=await fetch('/api/transferts');
    const transferts=await resT.json();
    const tbodyT=document.getElementById('transfert-body'); tbodyT.innerHTML='';
    transferts.forEach(t=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`
      <td>${t.code}</td><td>${t.originLocation}</td><td>${t.senderFirstName}</td><td>${t.senderPhone||'-'}</td>
      <td>${t.destinationLocation}</td><td>${t.receiverFirstName}</td><td>${t.receiverPhone||'-'}</td>
      <td>${t.amount}</td><td>${t.fees}</td><td>${t.received}</td>
      ...
      <button onclick="imprimerTransfert('${t._id}')">🖨</button>
      </td>`;
      tbodyT.appendChild(tr);
    });
    // Stocks
    const resS=await fetch('/api/stocks'); const stocks=await resS.json();
    const tbodyS=document.getElementById('stock-body'); tbodyS.innerHTML='';
    stocks.forEach(s=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`
      ...
      <td><button onclick="openStockModal('${s._id}')">✏️</button>
      <button onclick="deleteStock('${s._id}')">❌</button></td>`;
      tbodyS.appendChild(tr);
    });
    // Clients
    const resC=await fetch('/api/clients'); const clients=await resC.json();
    const tbodyC=document.getElementById('client-body'); tbodyC.innerHTML='';
    clients.forEach(c=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`
      <td>${c.lastName}</td><td>${c.firstName}</td><td>${c.phone}</td><td>${c.email||'-'}</td>
      <td>${c.kycVerified?'✅':'❌'}</td>
      <td><button onclick="openClientModal('${c._id}')">✏️</button>
      <button onclick="deleteClient('${c._id}')">❌</button></td>`;
      tbodyC.appendChild(tr);
    });
    // Rates
    const resR=await fetch('/api/rates'); const rates=await resR.json();
    const tbodyR=document.getElementById('rate-body'); tbodyR.innerHTML='';
    rates.forEach(r=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`<td>${r.from}</td><td>${r.to}</td><td>${r.rate}</td>
      <td><button onclick="openRateModal('${r._id}')">✏️</button>
      <button onclick="deleteRate('${r._id}')">❌</button></td>`;
...
// ================== API ROUTES ==================
app.get('/api/transferts', requireLogin, async(req,res)=>{
  const filters={};
  if(req.query.code) filters.code={$regex:req.query.code,$options:'i'};
  if(req.query.sender) filters.senderFirstName={$regex:req.query.sender,$options:'i'};
  if(req.query.receiver) filters.receiverFirstName={$regex:req.query.receiver,$options:'i'};
  if(req.query.currency) filters.currency=req.query.currency;
  if(req.query.status==='true') filters.retired=true;
  if(req.query.status==='false') filters.retired=false;
  if(req.query.dateFrom||req.query.dateTo){ filters.createdAt={};
    if(req.query.dateFrom) filters.createdAt.$gte=new Date(req.query.dateFrom);
    if(req.query.dateTo) filters.createdAt.$lte=new Date(req.query.dateTo+'T23:59:59');
  }
  res.json(await Transfert.find(filters).sort({createdAt:-1}));
});
app.get('/api/stocks', requireLogin, async(req,res)=>res.json(await StockHistory.find().sort({createdAt:-1})));
