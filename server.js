 ...
 * COMPLET : Dashboard avec modals CRUD et design responsive
 ******************************************************************/
...
app.get('/login',(req,res)=>{
  ...
  .login-container button:hover{background:#e67300;}</style></head>
  <body><div class="login-container"><h2>Connexion</h2>
  <form method="post">
...
app.get('/dashboard', requireLogin, async(req,res)=>{
  ...
  // HTML Dashboard complet avec tables, modals et recherche instantanée
  let html=`<html><head><meta name="viewport" content="width=device-width, initial-scale=1"><style>
  body{font-family:Arial;background:#f0f2f5;margin:0;padding:20px;}h2,h3,h4{margin-top:20px;color:#333;}a{margin-right:10px;text-decoration:none;color:#007bff;}a:hover{text-decoration:underline;}
  ...
  <table><tr><th>Code</th><th>Origine</th><th>Expéditeur</th><th>Téléphone</th><th>Destination</th><th>Destinataire</th><th>Téléphone</th><th>Montant</th><th>Frais</th><th>Reçu</th><th>Devise</th><th>Mode</th><th>Status</th><th>Actions</th></tr>`;
  ...
  <input id="t_senderPhone" placeholder="Téléphone">
...
// Rate
function openRateModal(id=null){currentRateId=id;document.getElementById('rateModal').style.display='flex';if(!id){r_from.value='';r_to.value='';r_rate.value='';return;}fetch('/rate/'+id).then(r=>r.json()).then(rat=>{r_from.value=rat.from;r_to.value=rat.to;r_rate.value=rat.rate;});}
function closeRateModal(){document.getElementById('rateModal').style.display='none';currentRateId=null;}
function saveRate(){const data={from:r_from.value,to:r_to.value,rate:Number(r_rate.value)};postData(currentRateId?'/rate/update/'+currentRateId:'/rate/create',data).then(()=>location.reload());}

// Delete
function deleteTransfert(id){if(confirm('Supprimer ce transfert?'))postData('/transfert/delete/'+id,{}).then(()=>location.reload());}
function deleteStock(id){if(confirm('Supprimer ce stock?'))postData('/stock/delete/'+id,{}).then(()=>location.reload());}
function deleteClient(id){if(confirm('Supprimer ce client?'))postData('/client/delete/'+id,{}).then(()=>location.reload());}
function deleteRate(id){if(confirm('Supprimer ce taux?'))postData('/rate/delete/'+id,{}).then(()=>location.reload());}

// Retrait
function retirerTransfert(id){if(confirm('Retirer ce transfert?'))postData('/transfert/retirer/'+id,{}).then(()=>location.reload());}
