...
const requirePhoneAuth = (req,res,next)=>{
  if(req.session.phoneUser) return next();
  res.redirect('/phone');
};
...
<form method="post">
<input name="phone" placeholder="Numéro de téléphone" required><br>
...
<div><label>Origine</label><select name="originLocation">${locations.map(v=>`<option>${v}</option>`).join('')}</select></div>
</div>
...
<div><label>Destination</label><select name="destinationLocation">${locations.map(v=>`<option>${v}</option>`).join('')}</select></div>
</div>
...
table{width:95%;margin:auto;border-collapse:collapse}th,td{border:1px solid #ccc;padding:8px;text-align:center}th{background:#007bff;color:white}button{padding:4px 8px;margin:2px;cursor:pointer}button.delete{background:#dc3545;color:white}button.print{background:#17a2b8;color:white}a{margin:2px;text-decoration:none}form{display:inline}
...
<h2>Liste des transferts</h2><a href="/menu">⬅ Menu</a> | <a href="/transferts/new">➕ Nouveau</a> | <a href="/transferts/pdf">📄 PDF</a><hr>`;
  for(let dest in grouped){
    let ta=0,tf=0,tr=0;
    html+=`<h3>Destination: ${dest}</h3><table><tr><th>Type</th><th>Expéditeur</th><th>Tél</th><th>Origine</th><th>Montant</th><th>Frais</th><th>Reçu</th><th>Destinataire</th><th>Tél</th><th>Code</th><th>Statut</th><th>Actions</th></tr>`;
    grouped[dest].forEach(t=>{
      ...
      html+=`<tr class="${t.retired?'retired':''}"><td>${t.userType}</td><td>${t.senderFirstName} ${t.senderLastName}</td><td>${t.senderPhone}</td><td>${t.originLocation}</td><td>${t.amount}</td><td>${t.fees}</td><td>${t.recoveryAmount}</td><td>${t.receiverFirstName} ${t.receiverLastName}</td><td>${t.receiverPhone}</td><td>${t.code}</td><td>${t.retired?'Retiré':'Non retiré'}<br>${histHtml}</td><td>
...
${t.retired?'':`<form method="post" action="/transferts/retirer"><input type="hidden" name="id" value="${t._id}"><select name="mode"><option>Espèces</option><option>Orange Money</option><option>Wave</option><option>Produit</option><option>Service</option></select><button>Retirer</button></form>`}
</td></tr>`;
    });
    html+=`<tr style="font-weight:bold;"><td colspan="4">Total ${dest}</td><td>${ta}</td><td>${tf}</td><td>${tr}</td><td colspan="5"></td></tr></table>`;
  }
  html+=`<h3>Total global</h3><table style="width:50%;margin:auto;"><tr style="font-weight:bold;"><td>Total Montant</td><td>${totalAmountAll}</td></tr><tr style="font-weight:bold;"><td>Total Frais</td><td>${totalFeesAll}</td></tr><tr style="font-weight:bold;"><td>Total Reçu</td><td>${totalReceivedAll}</td></tr></table></body></html>`;
  res.send(html);
});
